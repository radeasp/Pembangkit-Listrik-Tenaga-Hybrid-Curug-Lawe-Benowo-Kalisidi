...
export function initSensorModules(modules) {
  ...
  const initPromises = modules.map(async module => {
    ...
    // simpan instance agar bisa dipakai di fungsi lain
...
export function processData(module, data) {
  ...
  // PERBAIKAN: Ambil derived data dari buffer yang sudah dihitung di DataManager
  // TIDAK menghitung ulang untuk menghindari duplikasi
  switch (module) {
    case 'baterai':
      // Ambil nilai terakhir dari buffer net current
    ...
    case 'beban':
      // Ambil nilai DC power dari buffer
      ...
      // Ambil nilai AC power dari buffer
      const acPowerBuffer = manager.dataBuffers.get('ac_output_power') || [];
      if (acPowerBuffer.length > 0) {
        const latestAcPower = acPowerBuffer[acPowerBuffer.length - 1];
        processed.ac_output_power = latestAcPower.y;
      } else {
        // Fallback calculation
        const acVoltage = processed.ac_output_voltage || 0;
        const acCurrent = processed.ac_output_current || 0;
        processed.ac_output_power = acVoltage * acCurrent;
      }
      break;
      
    case 'dump_load':
      // Ambil picohydro charging power dari buffer
      ...
      // Ambil dumpload power dari buffer
...
export function startUpdating(module, renderFn) {
  ...
  const updateInterval = setInterval(async () => {
    try {
      ...
      // PERBAIKAN: Tidak memanggil processData yang bisa menyebabkan duplikasi
      ...
      if (manager) {
        ...
        // Tambahkan nilai terbaru dari derived parameters
...
// Tambahkan method untuk handle perubahan sumber data
export function handleDataSourceChange(newSource) {
    // Re-initialize semua modul dengan sumber baru
    if (DataManager.instances) {
        Object.values(DataManager.instances).forEach(async (manager) => {
            await manager.initialize();
        });
    }
}
...
document.addEventListener('datasourcechanged', (event) => {
    handleDataSourceChange(event.detail.source);
});