class DataManager {
    ...
    createDerivedBuffers() {
        const derivedParams = {
            ...
            'beban': ['dc_input_power', 'ac_output_power'],
            'dump_load': ['picohydro_charging_power', 'dumpload_power'],
    ...
    recalculateFromHistory() {
        switch (this.module) {
            ...
            case 'beban':
                ...
                this.calculatePowerFromHistory('ac_output_power', 'ac_output_voltage', 'ac_output_current');
                break;
                
            case 'dump_load':
                this.calculatePowerFromHistory('picohydro_charging_power', 'picohydro_voltage', 'picohydro_charging_current');
                this.calculatePowerFromHistory('dumpload_power', 'dumpload_voltage', 'dumpload_current');
                break;
        ...
        this.updateInterval = setInterval(async () => {
            try {
                ...
                if (data[this.module] && typeof data[this.module] === 'object') {
                    // PERBAIKAN: Gunakan timestamp yang konsisten untuk semua parameter
                    ...
                    // PERBAIKAN: Hitung derived data dengan timestamp yang sama
    ...
    // PERBAIKAN: Gunakan timestamp yang konsisten dan cek duplikasi
    calculateDerivedDataRealTime(rawData, timestamp) {
        switch (this.module) {
            ...
            case 'beban':
                ...
                // AC Output Power
                const acVoltage = rawData.ac_output_voltage || 0;
                const acCurrent = rawData.ac_output_current || 0;
                const acPower = acVoltage * acCurrent;
                this.addToBufferSafe('ac_output_power', timestamp, acPower);
                
                console.log(`Real-time dc_power: ${dcPower}W, ac_power: ${acPower}W at ${timestamp}`);
                break;
                
            case 'dump_load':
                // Picohydro Charging Power
    ...
    // PERBAIKAN: Helper yang aman untuk menambah data ke buffer dengan cek duplikasi
    addToBufferSafe(param, timestamp, value) {
        ...
        if (existingIndex !== -1) {
            // Update nilai yang sudah ada daripada menambah baru
    ...
    handleSourceChange(newSource) {
        // Reset buffer saat ganti sumber
        this.dataBuffers.clear();
        this.lastUpdateTimestamp = 0;
        ...
        console.log(`Data source changed to ${newSource} for module ${this.module}`);
    }
}

document.addEventListener('datasourcechanged', (event) => {
    const newSource = event.detail.source;
    // Update semua instance DataManager yang aktif
    if (DataManager.instances) {
        Object.values(DataManager.instances).forEach(manager => {
            manager.handleSourceChange(newSource);
        });
    }
});
